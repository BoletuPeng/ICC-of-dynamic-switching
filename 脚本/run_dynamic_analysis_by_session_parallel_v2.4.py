...
DMN-ECN动态功能分析主脚本 - 修正版

v2.4 关键修正:
- 修正Louvain共识实现，使用正确的MATLAB流程：
  1. 运行N次 modularity_louvain_und_sign (处理有正负权重的MTD矩阵)
  2. 使用 agreement_weighted(CC, QQ) 计算加权协议矩阵 (权重是Q值)
  3. 使用 consensus_und(D, tau, reps) 进行共识聚类
- 新增 --LouvTau 参数: 共识聚类的阈值 (默认: 0.1, 与MATLAB代码一致)
- 新增 --LouvReps 参数: 共识聚类的重复次数 (默认: 500, 与MATLAB代码一致)

参数说明:
- --Louv N: 每时间点运行Louvain算法的次数
  - N=1: 单次运行 (OSF实际代码行为)
  - N>1: 共识聚类 (论文声称的方法)
- --LouvTau: consensus_und的阈值参数 (默认0.1)
- --LouvReps: consensus_und的重复次数参数 (默认500)

输出目录命名:
    Louv1_Kmns100_noMotion_notSNR_fsavg6_ROI
    Louv500_tau0.1_reps500_Kmns200_yesMotion_yestSNR_MNI_VoxVert

使用方法:
    # 默认模式 (Louv=1, 单次运行，与OSF实际代码一致)
    python run_dynamic_analysis_by_session_parallel_v2.4.py \\
        ...
        --output_dir /path/to/dynamic_analysis \\
        --n_jobs 4

    # 论文声称模式 (Louv=500, 共识聚类)
    python run_dynamic_analysis_by_session_parallel_v2.4.py \\
        --roi_dir /path/to/roi_timeseries \\
        --behavior_file /path/to/behavior_data.csv \\
        --methods_dir /path/to/Methods \\
...
Version: 2.4 (with corrected Louvain consensus implementation)
"""
...
def load_roi_data_by_session(roi_dir, space='fsaverage6', level='ROI'):
    """
    按session加载ROI时间序列数据
    ...
    if level == 'ROI':
        ...
        file_prefix = 'rsfc_vertex' if space == 'fsaverage6' else 'rsfc_voxel'
    
    # 查找所有session文件
    session_files = sorted(glob.glob(os.path.join(data_dir, f'{file_prefix}_ses-*.mat')))
    
    if not session_files:
        session_files = sorted(glob.glob(os.path.join(data_dir, f'{file_prefix}_ses-*.npz')))
    
    if session_files:
        for f in session_files:
            basename = os.path.basename(f)
            parts = basename.replace('.mat', '').replace('.npz', '').split('_')
            ses_id = None
            for p in parts:
                if p.startswith('ses-'):
                    ses_id = p
            ...
            if f.endswith('.mat'):
                ...
                rsfc_raw = mat['rsfc']
                sub_ids_raw = mat['sub_ids']
            else:
                data = np.load(f, allow_pickle=True)
                rsfc_raw = data['rsfc']
                sub_ids_raw = data['sub_ids']
            
            rsfc_list = []
            n_subjects = rsfc_raw.shape[0] if hasattr(rsfc_raw, 'shape') else len(rsfc_raw)
            
            for i in range(n_subjects):
                subject_data = rsfc_raw[i]
                if isinstance(subject_data, np.ndarray):
                    rsfc_list.append(subject_data)
                else:
                    rsfc_list.append(np.array(subject_data))
            
            sub_ids_clean = []
            if isinstance(sub_ids_raw, np.ndarray):
                for s in sub_ids_raw:
                    if isinstance(s, np.ndarray):
                        s = str(s.flatten()[0]) if s.size > 0 else ''
                    sub_ids_clean.append(str(s))
            elif isinstance(sub_ids_raw, (list, tuple)):
                sub_ids_clean = [str(s) for s in sub_ids_raw]
            else:
                sub_ids_clean = [str(sub_ids_raw)] * n_subjects
            
            if len(rsfc_list) > 0 and len(rsfc_list) == len(sub_ids_clean):
                session_data[ses_id] = {
                    'rsfc': rsfc_list,
                    'sub_ids': sub_ids_clean
                }
...
def load_roi_info(roi_dir, space='fsaverage6', level='ROI'):
    """加载ROI信息"""
    ...
    if os.path.exists(info_file):
        ...
        if space == 'fsaverage6':
            space_info = full_info.get('fsaverage6', {})
            roi_info = {
                'n_dmn_lh': space_info.get('n_dmn_lh', 0),
                'n_dmn_rh': space_info.get('n_dmn_rh', 0),
                'n_ecn_lh': space_info.get('n_ecn_lh', 0),
                'n_ecn_rh': space_info.get('n_ecn_rh', 0),
                'space': 'fsaverage6',
                'level': level
            }
        else:
            space_info = full_info.get('mni', {})
            roi_info = {
    ...
    old_info_file = os.path.join(roi_dir, space, level, 'roi_info.json')
    if os.path.exists(old_info_file):
        with open(old_info_file, 'r') as f:
            return json.load(f)
...
def generate_output_dirname(louv, louv_tau, louv_reps, kmns, motion, tsnr, space, level):
    ...
    space_str = "fsavg6" if space == "fsaverage6" else space
    level_str = level
...
def run_single_analysis(roi_dir, behavior_file, methods_dir, output_dir,
    ...
    level_desc = "Vertex" if (level == "Vox_Vert" and space == "fsaverage6") else \
                 "Voxel" if (level == "Vox_Vert" and space == "MNI") else "ROI"
    ...
    session_data, all_sessions = load_roi_data_by_session(roi_dir, space, level)
    ...
    # 加载ROI信息
    roi_info = load_roi_info(roi_dir, space, level)
    if roi_info:
        if space == 'fsaverage6':
            n_dmn = roi_info.get('n_dmn_lh', 0) + roi_info.get('n_dmn_rh', 0)
            n_ecn = roi_info.get('n_ecn_lh', 0) + roi_info.get('n_ecn_rh', 0)
...
def run_analysis(roi_dir, behavior_file, methods_dir, output_dir,
    ...
    if space is not None and level is not None:
        ...
        combinations = [('fsaverage6', level), ('MNI', level)]
    else:
        combinations = []
    ...
    master_log.append(f"Dynamic Functional Analysis - v2.4 (Corrected Consensus)")
...
def main():
    parser = argparse.ArgumentParser(
        description='DMN-ECN Dynamic Functional Analysis v2.4 (Corrected Consensus)',
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
v2.4 Key Changes:
  - Corrected Louvain consensus implementation using MATLAB-equivalent method:
    1. Run N iterations of modularity_louvain_und_sign (handles positive/negative weights)
    2. Compute weighted agreement matrix using agreement_weighted(CC, QQ)
    3. Apply consensus_und(D, tau, reps) for final clustering
  - Added --LouvTau and --LouvReps parameters for consensus clustering

Examples:
  # Default mode (Louv=1, single run, matches OSF actual code)
  python run_dynamic_analysis_by_session_parallel_v2.4.py \\
      --roi_dir /path/to/roi_timeseries \\
      --behavior_file /path/to/behavior_data.csv \\
      --methods_dir /path/to/Methods \\
  ...
  # Paper-claimed mode (Louv=500, consensus clustering)
  python run_dynamic_analysis_by_session_parallel_v2.4.py \\
      --roi_dir /path/to/roi_timeseries \\
      --behavior_file /path/to/behavior_data.csv \\
      --methods_dir /path/to/Methods \\
  ...
  # With Motion and tSNR filtering
  python run_dynamic_analysis_by_session_parallel_v2.4.py \\
      --roi_dir /path/to/roi_timeseries \\
      --behavior_file /path/to/behavior_data.csv \\
      --methods_dir /path/to/Methods \\
...
Output folder naming:
  ...
  Louv500_tau0.1_reps500_Kmns200_yesMotion_yestSNR_MNI_VoxVert
    ...
    parser.add_argument('--roi_dir', required=True,
                       help='Directory containing ROI timeseries data')
    ...
    parser.add_argument('--space', choices=['fsaverage6', 'MNI'], default=None,
